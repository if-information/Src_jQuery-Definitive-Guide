$.extend({
    jigsaw: function(id, imgUri, limitWidth, breakCount, swapCount, completed) {
        ...
        if (!swapCount || swapCount <= 0) swapCount = breakCount * breakCount;
        if (!limitWidth || limitWidth <= 0) limitWidth = window.innerWidth;
        ...
        var verify = function() {
            ...
            if (success) {
                for (var i = 0; i < bricks.length; i++) { bricks[i].unbind('click'); }
                if (completed && completed != undefined) { completed.apply(this, arguments); }
